...
Using this mathematical intuition, the number of considerable containers can be narrowed down significantly in comparison to the brute-force <Eq math='O(length(height)^2)' /> approach. Starting with the widest container, new containers with minimum heights smaller than the current bounding height can be skipped.
