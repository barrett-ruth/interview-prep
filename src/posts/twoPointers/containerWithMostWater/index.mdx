...
The problem is asking us to maximize this area. While this can trivially be performed by enumerating all pairs in <Eq math='O(length(height)^2)' /> time, the key insight in finding an efficient solution lies in exploiting the problem constraints.
...
Using this mathematical intuition, the number of considerable containers can be narrowed down significantly in comparison to the <Eq math='O(length(height)^2)' /> <FootnoteRef description={<Code code={bruteforce} />}>brute-force approach</FootnoteRef>. Starting with the widest container, new containers with minimum heights smaller than the current bounding height can be skipped.

## Solving the Problem

Using the two pointers technique and the aforementioned logic, the solution nearly writes itself.

First, initialize the pointers <Eq math='l,r' /> to the widest positions and return value <FootnoteRef description={`If the list of heights is empty (no water is contained), 0 is a sensible default.`}><Eq math='area' /></FootnoteRef>:
